...
export default class EventStream {
  ...
  constructor(protected Config: ConfigContract, public logger: LoggerContract) {
  ...
  private corsHandler(req: IncomingMessage, res: ServerResponse) {
    ...
    res.setHeader("Access-Control-Expose-Headers", String(cors.exposeHeaders));
  ...
  async init(source: InstanceType<EventSourceContract>) {
    ...
    return function (
      ...
      if (this.hasValidAcceptHeaders(req.headers["accept"] || "")) {
        const isIE =
          req.headers["ua-cpu"] ||
          (req.headers["user-agent"] || "unknown").match(/Trident [\d]{1}/g) !==
            null;
        ...
        const dataListener = (payload: Partial<EventSourcePayload>) => {
          ...
          if (payload.event) {
            ...
            if (options.preferedEventName) {
              res.write(`event: ${options.preferedEventName || "message"}\n`);
            }
          }
